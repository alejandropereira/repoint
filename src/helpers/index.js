...
export const missingParams = (obj, attrs) =>
  attrs.filter((el) => el !== IS_COLLECTION).filter((el) => !R.contains(el, R.keys(obj)))
...
export const urlParamsTransformer = (url, params) => {
  ...
  return url.split('/')
    .map((token) =>
        pattern.test(token) ? token.replace(pattern, params[removeFirstChar(token)]) : token
    )
    .join('/')
}
